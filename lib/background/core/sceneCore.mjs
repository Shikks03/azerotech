// Canonical idle-background scene for the app — framework-agnostic: it takes a
// <canvas> plus options and returns a lifecycle handle. The app layers quality
// tiers + an FPS watchdog on top; a local sandbox could add HUD/capture.
//
// Uses bare specifiers ("three", "three/addons/...") that resolve under both a
// browser import map and the app's bundler (three maps ./addons/* -> ./examples/jsm/*).
...
export function createScene(canvas, options = {}) {
  ...
  // MSAA is redundant on high-DPR screens (the pixel density already anti-aliases),
  // so drop it there for free. Prefer the discrete GPU on dual-GPU laptops.
  const effectiveAA = opts.antialias && window.devicePixelRatio < 2;
  ...
  return {
    ...
    /** Render one frame at a fixed size and return it as a WebP blob (dev capture). */
